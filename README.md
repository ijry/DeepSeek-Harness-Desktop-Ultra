...
**dsh 自己的界面不在范围内**，那是上游的东西（见「项目边界」）。四个内置/自研插件的界面则跟着走：
外壳在拉起 dsh 时把语言写进子进程的 `DSH_DESKTOP_LANG`，插件的 host 半边读它，再通过各自的
接口发给浏览器半边。这条路只在**启动时**读一次，所以在设置里切完语言，要按一下「重启 dsh 服务」
插件才会跟着变——设置页会在切换后直接把那个按钮显示出来。
