...
- 同样支持暗黑模式，自动跟随系统主题。仓库面板是中英双语的；章鱼Git 目前只有中文，
  见「界面语言」。
...
用户的仓库上执行写操作（提交、重置、推送），默认装进每台机器都不合适。
...
仓库面板这四个插件的界面则跟着走：外壳在拉起 dsh 时把语言写进子进程的 `DSH_DESKTOP_LANG`，
...
**章鱼Git 还只有中文。** 它是双语化那一轮之后并行加进来的，没跟上；它自己也不打进安装包，
所以不影响外壳的双语体验。补齐是一件独立的事。
