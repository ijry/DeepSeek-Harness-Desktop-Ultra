 ...
 *   - build the React panel with Vite into lib/webview (skippable)
 *
 * The webview step is what this plugin shares with 鲨鱼数据库 and nothing else in the
 * repo: its UI is the reference app's React front end rather than a hand-written DOM
 * bundle, so it needs a real bundler. `--skip-webview` (or `AI_SWITCH_SKIP_WEBVIEW=1`)
 * leaves an existing `lib/webview` alone, which is what `npm run check` uses — a
 * syntax check should not cost a 20-second Vite build.
 *
 * @module dsh-plugin-ai-switch/scripts/build
 */
...
const skipWebview = process.argv.includes('--skip-webview') || process.env.AI_SWITCH_SKIP_WEBVIEW === '1'
