...
/** This plugin's own data directory: <DSH home>/plugins/dsh-plugin-automation/。 */
export function pluginDataPath(...segments) {
  return dshHomePath('plugins', 'dsh-plugin-automation', ...segments)
}
...
export function adoptLegacyData(legacyRelative, ...target) {
  ...
  try {
    ...
    console.warn(`[dsh-plugin-automation] 迁移旧数据 ${from} 失败，已跳过:`, error?.message ?? error)
  }
}
