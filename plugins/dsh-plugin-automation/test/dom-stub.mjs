 ...
 *
 * 复制自 dsh-plugin-canvas 的同名文件（同仓库、同许可），另加了三样这个面板要用的东西：
 * `createTextNode`、`Node`（client 的 el() 用 `instanceof Node` 区分节点与文本）、
 * 以及 globals 里的 `EventSource` / `URLSearchParams`。
 *
 ...
 * 测试通过。
 *
 * @module dsh-plugin-automation/test/dom-stub
 */
...
export class StubElement {
  ...
  dispatch(type, init = {}) {
    ...
    let node = this
    while (node !== null) {
      // 真实浏览器里 currentTarget 是正在处理事件的那个节点，随冒泡变化。
...
export function createStubDom(routes = {}) {
  ...
  const document = {
    ...
    createTextNode: (text) => {
      // 文本节点当成一个只有 _text 的元素：textContent 的 getter 正好返回它。
  ...
  return {
    ...
    globals: {
      ...
      // client 的 el() 用 `child instanceof Node` 区分节点与文本，事件处理用
      // `target instanceof Element`；替身里这三者同类。
      Node: StubElement,
      Element: StubElement,
      EventSource: StubEventSource,
      URLSearchParams,
