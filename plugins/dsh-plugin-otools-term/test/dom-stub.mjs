...
class El extends Node {
  ...
  append(...nodes) {
    for (const node of nodes) {
      ...
      // A <script src> appended to <head> would load in a browser. There is no
      // loader here, so it fails instead — which is the case the panel has to
      // survive: no xterm means a terminal pane that says so, not a crash.
      if (child instanceof El && child.localName === 'script' && child.attributes.has('src')) {
        setTimeout(() => child.dispatchEvent({ type: 'error' }), 0)
      }
    }
  }
...
export function installDom(options) {
  ...
  const window = {
    ...
    navigator: {
      language: 'zh-CN',
      platform: 'Win32',
      clipboard: { writeText: async () => undefined, readText: async () => 'pasted' },
    },
    // base64, the way a browser spells it.
    btoa: (text) => Buffer.from(String(text), 'binary').toString('base64'),
    atob: (text) => Buffer.from(String(text), 'base64').toString('binary'),
    // The terminal themes read design tokens off the document root.
    ...
    // Never connects, so the panel simply reports "not connected". `__emit` lets a
    // test hand it a frame as if the host had sent one.
    EventSource: class {
      constructor(url) {
        this.url = url
        this.readyState = 0
      ...
      __emit(type, data) {
        for (const handler of [...(this.handlers.get(type) ?? [])]) {
          handler({ type, data: data === undefined ? '' : JSON.stringify(data) })
        }
      }
