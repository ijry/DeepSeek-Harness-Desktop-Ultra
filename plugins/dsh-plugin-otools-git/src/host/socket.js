/**
 * The git panel's event socket: the SSE stream's frames on a WebSocket instead.
 *
 ...
 *
 * @module dsh-plugin-otools-git/host/socket
 */
...
export function createEventSocket(ctx, options) {
  ...
  const handler = (req, socket, head) => {
    ...
    // stream and watch what this panel does to the user's repositories.
    ...
    const consume = () => {
      for (;;) {
        ...
        // so nothing the browser sends can reach `git` through it.
      }
    }
