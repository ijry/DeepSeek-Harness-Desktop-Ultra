/**
 * Event-socket tests for dsh-plugin-repopanel: the hand-rolled RFC 6455 pieces
...
test('握手成功后先发 hello，broadcast 落到每个客户端', () => {
  ...
  assert.equal(ctx.registered.path, '/dsh-plugin-repopanel/socket')
  ...
  hub.broadcast('change', { revision: 8, kind: 'links-changed' })
  assert.equal(hub.size(), 2)
  assert.deepEqual(first.frames().at(-1), { event: 'change', data: { revision: 8, kind: 'links-changed' } })
  assert.deepEqual(second.frames().at(-1), { event: 'change', data: { revision: 8, kind: 'links-changed' } })
...
test('真实握手：Node 自带的 WebSocket 客户端能连上并收到 hello 与 change', async () => {
  ...
  hub.broadcast('change', { revision: 4, kind: 'settings-changed' })
  await received
  assert.deepEqual(frames, [
    { event: 'hello', data: { revision: 3 } },
    { event: 'change', data: { revision: 4, kind: 'settings-changed' } },
  ])
