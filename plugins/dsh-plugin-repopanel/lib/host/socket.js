/**
 * The repopanel event socket: the SSE stream's frames on a WebSocket instead.
 *
 ...
 *
 * @module dsh-plugin-repopanel/host/socket
 */
...
export function createEventSocket(ctx, options) {
  ...
  const handler = (req, socket, head) => {
    ...
    // stream and watch the panel's ledger change.
