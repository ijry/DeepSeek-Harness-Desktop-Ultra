...
/** This plugin's own data directory: <DSH home>/plugins/dsh-plugin-repopanel/。 */
export function pluginDataPath(...segments) {
  return dshHomePath('plugins', 'dsh-plugin-repopanel', ...segments)
}
...
export function adoptLegacyData(legacyRelative, ...target) {
  ...
  try {
    ...
    console.warn(`[dsh-plugin-repopanel] 迁移旧数据 ${from} 失败，已跳过:`, error?.message ?? error)
  }
}
