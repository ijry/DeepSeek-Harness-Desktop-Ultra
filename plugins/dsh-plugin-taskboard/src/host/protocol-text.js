...
export const CODEG_TASKBOARD_PROTOCOL = [
  ...
  '状态含义：todo=还没启动的待办；queued=已启动、在队列里（等空位，或会话已分配、等认领）；preparing=认领后准备中；running=执行中；',
  'awaiting_input=正在等你输入/决策；review=实现完成，待你验收；failed=失败可重试；done=你已验收完成。',
  '并发上限：看板有 settings.maxParallel（默认 3），同一时刻最多这么多张卡能占用会话；用户点「启动任务」只是把卡片送进队列，',
  '有空位就立刻由队列调度器发起 DSH 会话，没空位就停在「排队中」，空位一出现（其他卡交验/验收/失败/取消排队）自动发起。',
...
export const CODEG_TASKBOARD_PROTOCOL_EN = [
  ...
  'Status meanings: todo=not started yet; queued=started and in the queue (waiting for a slot, or holding a session awaiting a claim); preparing=claimed and getting ready; running=executing;',
  ...
  'Parallelism: the board has settings.maxParallel (default 3) — at most that many tasks may hold a session at once. Starting a task only enqueues it:',
  'the queue starts the DSH session immediately when a slot is free, otherwise the card waits at “queued” and starts as soon as a slot frees',
  '(another card handed off, accepted, failed, or left the queue).',
