/**
 * The canvas event socket: the SSE stream's frames on a WebSocket instead.
 *
 ...
 *
 * @module dsh-plugin-canvas/host/socket
 */
  ...
  const handler = (req, socket, head) => {
    ...
    const consume = () => {
      for (;;) {
        ...
        // so nothing the browser sends can reach the board through it.
      }
    }
