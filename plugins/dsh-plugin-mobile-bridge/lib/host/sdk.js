...
/** This plugin's own data directory: <DSH home>/plugins/dsh-plugin-mobile-bridge/。 */
export function pluginDataPath(...segments) {
  return dshHomePath('plugins', 'dsh-plugin-mobile-bridge', ...segments)
}
...
export function adoptLegacyData(legacyRelative, ...target) {
  ...
  try {
    ...
    console.warn(`[dsh-plugin-mobile-bridge] 迁移旧数据 ${from} 失败，已跳过:`, error?.message ?? error)
  }
}
